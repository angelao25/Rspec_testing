...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 10 and random_number is 3' do
      subject(:new_instance) { described_class.new(3)}

      it 'returns 7' do
        previous_step = 10
        result = new_instance.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 5' do
      subject(:new_instance2) { described_class.new(5)}
      # remove the 'x' before running this test
      it 'will return 7' do
        result = new_instance2.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 10' do
      subject(:new_instance3) { described_class.new(10)}
      # remove the 'x' before running this test
      it 'will return 7' do
        result = new_instance3.play
        expect(result).to eq(7)
      end
    end
    ...
    context 'when the random number is 15' do
      # remove the 'x' before running this test
      subject(:new_instance4) { described_class.new(15)}
      it 'will return 7' do
        result = new_instance4.play
        expect(result).to eq(7)
      end
    end
  end
