...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      ...
      subject(:new_instance) { described_class.new(5, '6')}
      # remove the 'x' before running this test
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      ...
      subject(:game_over) { described_class.new(5, '5', 3)}
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_over.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    context 'when count is 4 and over' do
      ...
      subject(:final_game) { described_class.new(5, '5', 4)}
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        getsome_phrase = "That was hard. It took you 4 guesses!\n"
        expect { final_game.final_message}.to output(getsome_phrase).to_stdout
      end
    end
  end
end
