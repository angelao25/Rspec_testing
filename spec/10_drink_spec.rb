...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      it 'is your choice of beverage' do
        expect(orange_juice.type).to eq("orange juice")
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(orange_juice.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:warm_coffee) { described_class.new("warm coffee", 6) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(warm_coffee).not_to be_full
      end
    end
  end
end
